...
public class CacheTestUtils {
     ...
     */
    public static Cache.Entry<byte[]> makeRandomCacheEntry(
            byte[] data, boolean isExpired, boolean needsRefresh) {
        Random random = new Random();
        Cache.Entry<byte[]> entry = new Cache.Entry<byte[]>();
        if (data != null) {
            entry.data = data;
        } else {
     ...
     */
    public static Cache.Entry<byte[]> makeRandomCacheEntry(byte[] data) {

...
public class RequestQueueTest extends InstrumentationTestCase {
    ...
    public void testAdd_requestProcessedInCorrectOrder() throws Exception {
        ...
        RequestQueue<byte[]> queue = new RequestQueue<byte[]>(new NoCache<byte[]>(), network, 1, mDelivery);

        for (Request<byte[],?> request : makeRequests(requestsToMake)) {
            queue.add(request);
        }
    ...
    public void testAdd_dedupeByCacheKey() throws Exception {
        ...
        DelayedRequest req2 = new DelayedRequest(0, parsed, delivered) {
            @Override
            protected Response<byte[],Object> parseNetworkResponse(NetworkResponse<byte[]> response) {
        ...
        RequestQueue<byte[]> queue = new RequestQueue<byte[]>(new NoCache<byte[]>(), network, 3, mDelivery);
        queue.add(req1);
        queue.add(req2);
        queue.start();
    ...
    private class OrderCheckingNetwork implements Network<byte[]> {
        ...
        @Override
        public NetworkResponse<byte[]> performRequest(Request<byte[],?> request) {
            ...
            mSemaphore.release();
            return new NetworkResponse<byte[]>(new byte[16], 16);
        }
    }

    private class DelayedRequest extends Request<byte[],Object> {

...
public class ImageRequestTest extends InstrumentationTestCase {
    ...
    public void testParseNetworkResponse_resizing() throws Exception {
        ...
        NetworkResponse<byte[]> jpeg = new NetworkResponse<byte[]>(jpegBytes, jpegBytes.length);
    ...
    private void verifyResize(NetworkResponse<byte[]> networkResponse, int maxWidth, int maxHeight,
            int expectedWidth, int expectedHeight) {
        ImageRequest request = new ImageRequest(
                "", null, maxWidth, maxHeight, Config.RGB_565, null);
        Response<byte[],Bitmap> response = request.parseNetworkResponse(networkResponse);
